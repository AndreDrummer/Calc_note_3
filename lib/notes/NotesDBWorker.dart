...
class NotesDBWorker {
  ...
  Future<Note> readNote(int inID) async {
    Database db = await database;
    List<Map<String, dynamic>> rec = await db.query(
      "notes",
      where: "id = ?",
      whereArgs: [inID],
    );
  ...
  Future<List<Map<String, dynamic>>> getAll() async {
    Database db = await database;
    List<Map<String, dynamic>> recs = await db.query("notes");
    List<Map<String, dynamic>> returnedList = recs.isNotEmpty ? recs.map((e) => noteFromMap(e)).toList() : [];
    return returnedList;
  }
}
